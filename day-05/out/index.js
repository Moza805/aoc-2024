...
const part1 = () => {
    ...
    for (let updateIndexString in updates) {
        ...
        while (reversedPages.length > 0 && valid) {
            ...
            const applicableRules = rules.filter((rule) => rule.page === page && update.pages.includes(rule.precededBy));
            const requiredPages = applicableRules.map((rule) => rule.precededBy);
            if (!requiredPages.every((requiredPage) => remainingPages.includes(requiredPage))) {
                valid = false;
                break;
            }
...
const part2 = (invalidUpdates) => {
    ...
    for (let update of invalidUpdates) {
        ...
        let applicableRules = rules.filter((rule) => update.pages.includes(rule.page) &&
            update.pages.includes(rule.precededBy));
        const firstPages = update.pages.filter((page) => applicableRules.some((rule) => rule.precededBy === page) &&
            applicableRules.every((rule) => rule.page !== page));
        const lastPages = update.pages.filter((page) => applicableRules.some((rule) => rule.page === page) &&
        ...
        const firstRule = applicableRules.filter((rule) => rule.precededBy === firstPage);
        console.log(firstRule);
    }
};
const part1Result = part1();
console.log(part1Result.validUpdates.reduce((aggregate, update) => (aggregate += update.pages[Math.floor(update.pages.length / 2)]), 0));
